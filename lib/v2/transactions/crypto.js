...
// default is smartholdem mainnet
// var networkVersion = 0x3f;
var networkVersion = 0x1e; //dev
...
function getBytes(transaction) {
	...
	bb.writeByte(transaction.network); // smartholdem = 0x3f, testnet = 0x41
...
function getKeys(secret, network) {
	var ecpair = ECPair.fromSeed(secret, network || networks.testnet);
...
function validateAddress(address, version){
	...
	try {
		...
		console.log('decode 0 validate:' + decode[0] + ' v '+version);

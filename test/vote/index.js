...
describe("vote.js", function () {
    ...
    describe("#createVote", function () {
        var createVote = vote.createVote,
            vt = null,
            publicKey = smartholdem.crypto.getKeys("secret").publicKey,
        ...
        it("should create vote from ecpair", function () {
            ...
            var secondSecretKey = smartholdem.ECPair.fromSeed("second secret");
        ...
        it("should create vote from wif", function () {
            var secretKey = smartholdem.ECPair.fromWIF("ejeMvgBZjLCK3Gy5XKUKcNWuftvt5CShUxmgmWHECEPqLz9kBxmM", NETWORKS.smartholdem);
            secretKey.publicKey = secretKey.getPublicKeyBuffer().toString("hex");

            var tx = createVote(secretKey, publicKeys);
            (tx).should.be.ok;
            (tx).should.be.type("object");
            (tx).should.have.property("recipientId").and.be.type("string").and.be.equal("SQjxubhNfRnXUAfD5p339iXBd6Vrt7rq6H");
        });

        it("should be deserialised correctly", function () {
            var deserialisedTx = smartholdem.crypto.fromBytes(smartholdem.crypto.getBytes(vt).toString("hex"));
        ...
        describe("returned vote", function () {
            ...
            it("should have recipientId string equal to sender", function () {
                (vt).should.have.property("recipientId").and.be.type("string").and.equal(smartholdem.crypto.getAddress(publicKey))
            });
            ...
            it("should be signed correctly", function () {
                var result = smartholdem.crypto.verify(vt);
                (result).should.be.ok;
            });

            it("should be second signed correctly", function () {
                var result = smartholdem.crypto.verifySecondSignature(vt, smartholdem.crypto.getKeys("second secret").publicKey);
                (result).should.be.ok;
            });
